...
namespace cm{
  ...
  namespace detail {
    ...
    protected:
      ...
      template<class U>
      void m_inplace_construct(U &&value) {
        m_engaged = true;
        new(&m_storage) T(std::forward<U>(value));
      }

      template<class U>
      void m_assign_or_construct(U &&value) {
        if (m_is_engaged())
      ...
      const T &m_get_value() const &{ return reinterpret_cast<const T &>(m_storage); }
    ...
    protected:
      void m_inplace_construct(T &value) {
        m_storage = std::addressof(value);
      }

      void m_assign_or_construct(T &value) {
        m_inplace_construct(value);
      }
...
template <class T>
class optional : public detail::optional_base_storage<T>
{
...
public:
  ...
  auto value() const && -> detail::constify<decltype(std::move(*this).base::m_get_value())>
  {
    m_check_access();
    return std::move(*this).base::m_get_value();
  }
  ...
  {
    return has_value()
      ? std::move(*this).base::m_get_value()
      : static_cast<T>(std::forward<U>(default_value));
  }

private:
  void m_check_access() const { if (!has_value()) throw bad_optional_access(); }
};
}
